...
# goal is to use the Logistic, DNN, and CNN methodologies with softmax to classify between one of the 7 face expressions
...
def DeepNeuralNet(X_Features,Y_predict,X_Features_Validation,Y_predict_Validation):
    ...
    # creating an Indicator one-hot-encoding scheme for this data
    Y_Indicator = np.zeros((len(Y_predict), len(set(Y_predict))))
    
    # Now fill in the one-hot-encoder values with the rlevant expression bit values set to 1
    ...
    ## initalize the weights - gaussian weights and also remeber to normalize
    ...
    for i in xrange(epochs):        
        ...
        # look for proof from Dr. Minje Kim material - Indiana university  
        ### Gradient descent
